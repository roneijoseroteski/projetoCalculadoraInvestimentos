...
function createTableHeader(tableReference, columnsArray) {
  ...
  // ["bg-blue-900", "text-white", "text-lg", "font-bold"].forEach((className) => {
  //   headerRow.classList.add(className);
  // });

  for (const tableColumnObject of columnsArray) {
    const headerElement = /*html*/ `<th class="text-center">${tableColumnObject.columnLabel}</th>`;
    headerRow.innerHTML += headerElement; // aqui garante que cada volta do loop, nos acrecentemos um th ao nosso tr
  }
